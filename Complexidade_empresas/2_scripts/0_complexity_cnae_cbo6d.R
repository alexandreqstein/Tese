...
foreach( k = seq_along(files),
    ...
    muns_zero <- data %>%
        ...
        ungroup() %>%
        filter(total < 202) %>% #removed sectors with less then 100 jobs
        pull(country)
    ...
    saveRDS(regions_complexity, str_c("3_results/6d_ECI", anos[k], ".RDS"))
    saveRDS(product_complexity, str_c("3_results/6d_PCI", anos[k], ".RDS"))
